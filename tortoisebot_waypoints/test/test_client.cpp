...
public:
  ...
  WaypointTest() {
    ...
    this->client_ptr_ = rclcpp_action::create_client<WaypointAction>( node, "/tortoisebot_as");

    _sub_odom = node-> create_subscription<nav_msgs::msg::Odometry>("/odom", 1, std::bind(&WaypointTest::odom_callback, this, std::placeholders::_1));
...
private:
  ...
  geometry_msgs::msg::Point position;
    ...
    void goal_response_callback(const std::shared_ptr<GoalHandleWaypoint> goal_handle)
        {
            if (!goal_handle) {
